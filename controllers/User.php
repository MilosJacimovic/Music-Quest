...
class User extends CI_Controller {
    ...
    <div class=\"collapse\" id=\"collapseInfo1\">
        <div class=\"card card-body\" id=\"textPart","\"><div class=\"accrej\">
                <br><button type=\"button\" class=\"btn btn-success btn-sm\" onClick=\"openCooperation(",")\">View cooperation</button>
                </div>
            </div>
      </div>");
    ...
    public function remapToCooperation($musName, $musOrg) {
        ...
        $this->load->view("templates/headerGost.php", ["currentPage"=>"cooperationStylesheet.css", "message"=>$message, "controller"=>"User", "user"=>$user, "resources"=>$resources]);
        $this->load->view($page, ["currentPage"=>"cooperationStylesheet.css", "message"=>$message, "controller"=>"User", "user"=>$user, "resources"=>$resources]);
    ...
    public function openNotificationsPage() {
        ...
        foreach ($coops->result() as $a) {
            ...
            if ($a->Status == "PENDING" && $a->IDReply != $user->IDUser) {
                ...
                $fillString.=$k.$this->acceptedArray[1].$otherUser->Name.$this->acceptedArray[2].$a->Date.$this->acceptedArray[3].$k.$this->acceptedArray[4].$a->IDCoop.$this->acceptedArray[5];
                $resources[$k++]=$fillString;
            }
        ...
        for ($i=0; $i<count($result); $i++) {
            ...
            if ($i==0) {
                $fillString.="<div class=\"carousel-item active\">
                    <a href=\"".site_url()."/User/remapToCooperation/".$userMus->Name."/".$userOrg->Name."\">
    ...
    public function searchUsers() {
        ...
        foreach ($result->result() as $row) {
            ...
            $stringToReturn.= "<div class=\"col-lg-3 col-md-4 col-sm-6\">
                <div class=\"card\">
                    <a href=\"".base_url()."index.php/User/remapTo".$type."/".$row->Username."\"><img src=../../".$row->ProfilePicture." class=\"card-img-top\" alt=\"Profile picture not uploaded!\"></a>
